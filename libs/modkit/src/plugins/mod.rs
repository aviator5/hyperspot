    ...
    async fn resolve_called_once_returns_same_str() {
        ...
        let id_a = selector
            .get_or_init(|| async move {
                ...
                Ok::<_, std::convert::Infallible>(
                    "gts.cf.modkit.plugins.plugin.v1~cf.core.test.plugin.v1~a.test._.plugin.v1"
                        .to_owned(),
                )
            })
            .await
        ...
        let id_b = selector
            .get_or_init(|| async move {
                ...
                Ok::<_, std::convert::Infallible>(
                    "gts.cf.modkit.plugins.plugin.v1~cf.core.test.plugin.v1~b.test._.plugin.v1"
                        .to_owned(),
                )
            })
            .await
            ...
            handles.push(tokio::spawn(async move {
                selector
                    .get_or_init(|| async {
                        ...
                        Ok::<_, std::convert::Infallible>(
                            "gts.cf.modkit.plugins.plugin.v1~cf.core.test.plugin.v1~concurrent.test._.plugin.v1"
                                .to_owned(),
                        )
                    })
                    .await
