...
//!
//! Every modkit plugin registers a well-known instance of this schema in
//! `types-registry` so consumers can discover and resolve plugins at runtime.
...
pub struct PluginV1<P: gts::GtsSchema> {
    /// Full GTS instance ID for this plugin instance.
...
impl<P: gts::GtsSchema + gts::GtsSerialize + Default> PluginV1<P> {
    ...
    /// plugin specs declared through `#[modkit_gts::gts_schema(base = PluginV1, ...)]`
    /// get a `Default` impl auto-emitted by the macro, so the caller only
    /// specifies the type once — in the turbofish.
    ///
    ...
    /// `gts.cf.modkit.plugins.plugin.v1~cf.core.authn_resolver.plugin.v1~hyperspot.builtin.static_authn_resolver.plugin.v1`).
