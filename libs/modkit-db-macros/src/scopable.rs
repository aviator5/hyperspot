...
const PROP_OWNER_TENANT_ID: &str = "owner_tenant_id";
const PROP_RESOURCE_ID: &str = "id";
const PROP_OWNER_ID: &str = "owner_id";

/// Reserved property names that must not be used in `pep_prop(...)`.
const RESERVED_PROPERTIES: &[(&str, &str)] = &[
    (PROP_OWNER_TENANT_ID, "tenant_col"),
    (PROP_RESOURCE_ID, "resource_col"),
    (PROP_OWNER_ID, "owner_col"),
];
...
fn generate_resolve_property(config: &SecureConfig, span: Span) -> TokenStream {
    ...
    if let Some((col_name, _)) = &config.tenant_col {
        ...
        arms.push(quote! {
            #PROP_OWNER_TENANT_ID => ::core::option::Option::Some(Self::Column::#col_ident),
        });
    }
    ...
    if let Some((col_name, _)) = &config.resource_col {
        ...
        let col_ident = syn::Ident::new(&col_variant, span);
        arms.push(quote! {
            #PROP_RESOURCE_ID => ::core::option::Option::Some(Self::Column::#col_ident),
        });
    }
    ...
    if let Some((col_name, _)) = &config.owner_col {
        ...
        let col_ident = syn::Ident::new(&col_variant, span);
        arms.push(quote! {
            #PROP_OWNER_ID => ::core::option::Option::Some(Self::Column::#col_ident),
        });
    }
