...
mod integration_tests {
    ...
    fn test_access_scope_builders() {
        ...
        assert_eq!(scope.all_values_for(properties::OWNER_TENANT_ID), &[tid]);
        ...
        assert_eq!(scope.all_values_for(properties::RESOURCE_ID), &[rid]);

        let scope = AccessScope::for_tenants_and_resources(vec![tid], vec![rid]);
        assert_eq!(scope.all_values_for(properties::OWNER_TENANT_ID), &[tid]);
        assert_eq!(scope.all_values_for(properties::RESOURCE_ID), &[rid]);
    }
    ...
    fn test_or_constraints() {
        ...
        assert!(scope.contains_value(properties::OWNER_TENANT_ID, t1));
        assert!(scope.contains_value(properties::OWNER_TENANT_ID, t2));
        assert!(scope.contains_value(properties::RESOURCE_ID, r1));
        // all_values_for collects from all constraints
        assert_eq!(scope.all_values_for(properties::OWNER_TENANT_ID).len(), 2);
    }
    ...
    fn test_has_property() {
        ...
        assert!(scope.has_property(properties::OWNER_TENANT_ID));
        assert!(!scope.has_property(properties::RESOURCE_ID));
    }
}
