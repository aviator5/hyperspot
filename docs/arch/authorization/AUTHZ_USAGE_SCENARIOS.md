...
PEP doesn't have tenant_closure. PEP fetches the resource first, then asks PDP for constraints based on resource attributes. PDP returns constraints (e.g., `eq(owner_tenant_id, T2)`), which PEP can enforce via in-memory validation against the prefetched record or via a second scoped query (it's up to the PEP developer to choose the approach).
...
PDP validates that T2 is in T1's subtree and returns constraints:
...
PEP compiles constraints and validates the prefetched record:
- **In-memory validation:** Check that the prefetched `owner_tenant_id` matches the scope. If it matches → return task. If not → **404 Not Found**.
- **Alternatively:** PEP can issue a second scoped query (`SELECT ... WHERE id = 'task-456' AND owner_tenant_id = 'T2'`) — module developer's choice.
- Resource not found in Step 1 → **404 Not Found**

**Why no TOCTOU concern in the case of in-memory validation:** For GET, the "use" is returning data to the client. Even if `owner_tenant_id` changed between check and response, no security violation occurs — the client either gets data they had access to at query time, or gets 404. For mutations (UPDATE/DELETE), see S10.
...
**For reads (S09):** No second query needed. PEP fetches the resource, asks PDP for decision, and returns the already-fetched data if allowed.
