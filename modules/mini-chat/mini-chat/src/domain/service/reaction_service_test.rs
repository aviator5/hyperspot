...
/// (no `owner_id`). This is needed because message and reaction entities
/// use `no_owner`, so scopes containing `owner_id` filters would fail-closed
/// to deny-all during query resolution.
