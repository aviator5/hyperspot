...
///
/// Holds ALL `OTel` instruments (~87 total). Active (P0+P1) instruments
/// implement the port trait. Deferred instruments are `#[allow(dead_code)]`
/// fields with `// deferred:` comments — they compile but produce no
/// runtime overhead until wired.
pub struct MiniChatMetricsMeter {
    // ── P0: Streaming & UX Health ──────────────────────────────────────
    stream_started_total: Counter<u64>,
    stream_completed_total: Counter<u64>,
    stream_failed_total: Counter<u64>,
    stream_disconnected_total: Counter<u64>,
    ...
    // ── P0: Turn Mutations ─────────────────────────────────────────────
    turn_mutation_total: Counter<u64>,
    turn_mutation_latency_ms: Histogram<f64>,

    // ── P0: Audit Emission Health ──────────────────────────────────────
    audit_emit_total: Counter<u64>,
    audit_emit_latency_ms: Histogram<f64>,

    // ── P1: Quota Enforcement ──────────────────────────────────────────
    quota_preflight_total: Counter<u64>,
    quota_reserve_total: Counter<u64>,
    quota_commit_total: Counter<u64>,
    quota_overshoot_total: Counter<u64>,
    quota_estimated_tokens: Histogram<f64>,
    quota_actual_tokens: Histogram<f64>,

    // ── P1: Streaming Incomplete ───────────────────────────────────────
    stream_incomplete_total: Counter<u64>,

    // ── P1: Cancellation ───────────────────────────────────────────────
    cancel_requested_total: Counter<u64>,
    cancel_effective_total: Counter<u64>,
    time_to_abort_ms: Histogram<f64>,
    streams_aborted_total: Counter<u64>,

    // ── P1: Attachment Upload ──────────────────────────────────────────
    attachment_upload_total: Counter<u64>,
    ...
    #[allow(dead_code)]
    outbox_enqueue_total: Counter<u64>,
    #[allow(dead_code)]
    outbox_dispatch_total: Counter<u64>,
    ...
    #[allow(dead_code)]
    retrieval_zero_hit_total: Counter<u64>,
    #[allow(dead_code)]
    indexed_chunks_per_chat: Histogram<f64>,
    #[allow(dead_code)]
    upload_rejected_total: Counter<u64>,
    #[allow(dead_code)]
    vector_stores_per_user: Histogram<f64>,
    #[allow(dead_code)]
    context_truncation_total: Counter<u64>,

    // ── P3: Cleanup job (deferred: cleanup job not implemented) ──
    #[allow(dead_code)]
    cleanup_job_runs_total: Counter<u64>,
    #[allow(dead_code)]
    cleanup_attempts_total: Counter<u64>,
    #[allow(dead_code)]
    cleanup_orphan_found_total: Counter<u64>,
    #[allow(dead_code)]
    cleanup_orphan_fixed_total: Counter<u64>,
    #[allow(dead_code)]
    ...
    #[allow(dead_code)]
    summary_regen_total: Counter<u64>,
    #[allow(dead_code)]
    summary_fallback_total: Counter<u64>,

    // ── P3: Image quota (deferred: image quota not implemented) ──
    #[allow(dead_code)]
    quota_image_commit_total: Counter<u64>,

    // ── P3: Idempotent replay (deferred: idempotent replay not implemented) ──
    #[allow(dead_code)]
    stream_replay_total: Counter<u64>,

    // ── Low-priority deferred ──────────────────────────────────────────
    #[allow(dead_code)]
    quota_tier_downgrade_total: Counter<u64>, // deferred: tier downgrade logic doesn't exist yet
    #[allow(dead_code)]
    quota_negative_total: Counter<u64>, // deferred: negative balance alerting
    #[allow(dead_code)]
    audit_redaction_hits_total: Counter<u64>, // deferred: redaction patterns not configurable
    #[allow(dead_code)]
    media_rejected_total: Counter<u64>, // deferred: overlaps with attachment_upload_total
    #[allow(dead_code)]
    unknown_error_code_total: Counter<u64>, // deferred: cardinality risk
    #[allow(dead_code)]
    credits_overflow_total: Counter<u64>, // deferred: billing edge case
}
...
impl MiniChatMetricsMeter {
    ...
    pub fn new(meter: &Meter, prefix: &str) -> Self {
        Self {
            // ── P0: Streaming ──────────────────────────────────────────
            stream_started_total: meter
                .u64_counter(format!("{prefix}_stream_started_total"))
                .with_description("Streams initiated")
                .build(),
            stream_completed_total: meter
                .u64_counter(format!("{prefix}_stream_completed_total"))
                .with_description("Streams completed successfully")
                .build(),
            stream_failed_total: meter
                .u64_counter(format!("{prefix}_stream_failed_total"))
                .with_description("Streams that ended with an error")
                .build(),
            stream_disconnected_total: meter
                .u64_counter(format!("{prefix}_stream_disconnected_total"))
            ...
            // ── P0: Turn Mutations ─────────────────────────────────────
            turn_mutation_total: meter
                .u64_counter(format!("{prefix}_turn_mutation_total"))
            ...
            // ── P0: Audit ──────────────────────────────────────────────
            audit_emit_total: meter
                .u64_counter(format!("{prefix}_audit_emit_total"))
                .with_description("Audit event emissions")
                .build(),
            audit_emit_latency_ms: meter
                .f64_histogram(format!("{prefix}_audit_emit_latency_ms"))
                .with_description("Audit emission latency (ms)")
                .build(),

            // ── P1: Quota ──────────────────────────────────────────────
            quota_preflight_total: meter
                .u64_counter(format!("{prefix}_quota_preflight_total"))
                .with_description("Quota preflight decisions")
                .build(),
            quota_reserve_total: meter
                .u64_counter(format!("{prefix}_quota_reserve_total"))
                .with_description("Quota reserves written")
                .build(),
            quota_commit_total: meter
                .u64_counter(format!("{prefix}_quota_commit_total"))
                .with_description("Quota commits (actual usage)")
                .build(),
            quota_overshoot_total: meter
                .u64_counter(format!("{prefix}_quota_overshoot_total"))
            ...
            stream_incomplete_total: meter
                .u64_counter(format!("{prefix}_stream_incomplete_total"))
                .with_description("Streams that ended incomplete (e.g. max_output_tokens)")
                .build(),

            // ── P1: Cancellation ───────────────────────────────────────
            cancel_requested_total: meter
                .u64_counter(format!("{prefix}_cancel_requested_total"))
                .with_description("Cancel requests received")
                .build(),
            cancel_effective_total: meter
                .u64_counter(format!("{prefix}_cancel_effective_total"))
            ...
            streams_aborted_total: meter
                .u64_counter(format!("{prefix}_streams_aborted_total"))
                .with_description("Streams aborted by trigger type")
                .build(),

            // ── P1: Attachment ─────────────────────────────────────────
            attachment_upload_total: meter
                .u64_counter(format!("{prefix}_attachment_upload_total"))
            ...
            provider_requests_total: meter
                .u64_counter(format!("{prefix}_provider_requests_total"))
                .with_description("Provider API requests")
                .build(),
            provider_errors_total: meter
                .u64_counter(format!("{prefix}_provider_errors_total"))
            ...
            provider_latency_ms: meter
                ...
                .build(),
            oagw_retries_total: meter
                .u64_counter(format!("{prefix}_oagw_retries_total"))
            ...
            oagw_upstream_latency_ms: meter
                ...
                .build(),
            oagw_circuit_open_total: meter
                .u64_counter(format!("{prefix}_oagw_circuit_open_total"))
                .with_description("OAGW circuit breaker opens")
                .build(),

            // deferred: waiting for modkit_db::outbox integration
            outbox_dead_total: meter
                .u64_counter(format!("{prefix}_outbox_dead_total"))
            ...
            outbox_oldest_pending_age_seconds: meter
                ...
                .build(),
            outbox_enqueue_total: meter
                .u64_counter(format!("{prefix}_outbox_enqueue_total"))
                .with_description("Outbox events enqueued")
                .build(),
            outbox_dispatch_total: meter
                .u64_counter(format!("{prefix}_outbox_dispatch_total"))
            ...
            db_errors_total: meter
                .u64_counter(format!("{prefix}_db_errors_total"))
            ...
            time_from_ui_disconnect_to_cancel_ms: meter
                ...
                .build(),
            cancel_orphan_total: meter
                .u64_counter(format!("{prefix}_cancel_orphan_total"))
                .with_description("Orphaned cancel events")
                .build(),

            // deferred: tool call execution not implemented
            tool_calls_total: meter
                .u64_counter(format!("{prefix}_tool_calls_total"))
                .with_description("Tool call invocations")
                .build(),
            tool_call_limited_total: meter
                .u64_counter(format!("{prefix}_tool_call_limited_total"))
            ...
            web_search_latency_ms: meter
                ...
                .build(),
            web_search_disabled_total: meter
                .u64_counter(format!("{prefix}_web_search_disabled_total"))
            ...
            citations_count: meter
                ...
                .build(),
            citations_by_source_total: meter
                .u64_counter(format!("{prefix}_citations_by_source_total"))
            ...
            retrieval_chunks_returned: meter
                ...
                .build(),
            retrieval_zero_hit_total: meter
                .u64_counter(format!("{prefix}_retrieval_zero_hit_total"))
            ...
            indexed_chunks_per_chat: meter
                ...
                .build(),
            upload_rejected_total: meter
                .u64_counter(format!("{prefix}_upload_rejected_total"))
            ...
            vector_stores_per_user: meter
                ...
                .build(),
            context_truncation_total: meter
                .u64_counter(format!("{prefix}_context_truncation_total"))
                .with_description("Context truncation events")
                .build(),

            // deferred: cleanup job not implemented
            cleanup_job_runs_total: meter
                .u64_counter(format!("{prefix}_cleanup_job_runs_total"))
                .with_description("Cleanup job runs")
                .build(),
            cleanup_attempts_total: meter
                .u64_counter(format!("{prefix}_cleanup_attempts_total"))
                .with_description("Cleanup attempts")
                .build(),
            cleanup_orphan_found_total: meter
                .u64_counter(format!("{prefix}_cleanup_orphan_found_total"))
                .with_description("Orphaned resources found during cleanup")
                .build(),
            cleanup_orphan_fixed_total: meter
                .u64_counter(format!("{prefix}_cleanup_orphan_fixed_total"))
            ...
            summary_regen_total: meter
                .u64_counter(format!("{prefix}_summary_regen_total"))
                .with_description("Summary regeneration events")
                .build(),
            summary_fallback_total: meter
                .u64_counter(format!("{prefix}_summary_fallback_total"))
                .with_description("Summary fallback events")
                .build(),

            // deferred: image quota not implemented
            quota_image_commit_total: meter
                .u64_counter(format!("{prefix}_quota_image_commit_total"))
                .with_description("Image quota commits")
                .build(),

            // deferred: idempotent replay not implemented
            stream_replay_total: meter
                .u64_counter(format!("{prefix}_stream_replay_total"))
                .with_description("Stream replay events")
                .build(),

            // deferred: low-priority
            quota_tier_downgrade_total: meter
                .u64_counter(format!("{prefix}_quota_tier_downgrade_total"))
                .with_description("Quota tier downgrades")
                .build(),
            quota_negative_total: meter
                .u64_counter(format!("{prefix}_quota_negative_total"))
                .with_description("Negative quota balance events")
                .build(),
            audit_redaction_hits_total: meter
                .u64_counter(format!("{prefix}_audit_redaction_hits_total"))
                .with_description("Audit redaction hits")
                .build(),
            media_rejected_total: meter
                .u64_counter(format!("{prefix}_media_rejected_total"))
                .with_description("Media files rejected")
                .build(),
            unknown_error_code_total: meter
                .u64_counter(format!("{prefix}_unknown_error_code_total"))
                .with_description("Unknown error codes encountered")
                .build(),
            credits_overflow_total: meter
                .u64_counter(format!("{prefix}_credits_overflow_total"))
...
impl MiniChatMetricsPort for MiniChatMetricsMeter {
    ...
    fn record_stream_completed(&self, provider: &str, model: &str) {
        self.stream_completed_total.add(
            1,
            &[
                KeyValue::new("provider", provider.to_owned()),
                KeyValue::new("model", model.to_owned()),
    ...
    fn record_stream_disconnected(&self, stage: &str) {
        self.stream_disconnected_total
            .add(1, &[KeyValue::new("stage", stage.to_owned())]);
    }
    ...
    fn record_turn_mutation_latency_ms(&self, op: &str, ms: f64) {
        self.turn_mutation_latency_ms
            .record(ms, &[KeyValue::new("op", op.to_owned())]);
    }

    // ── P0: Audit ──────────────────────────────────────────────────────

    fn record_audit_emit(&self, result: &str) {
        self.audit_emit_total
            .add(1, &[KeyValue::new("result", result.to_owned())]);
    }

    fn record_audit_emit_latency_ms(&self, ms: f64) {
        self.audit_emit_latency_ms.record(ms, &[]);
    }

    // ── P1: Quota ──────────────────────────────────────────────────────

    fn record_quota_preflight(&self, decision: &str, model: &str, tier: &str) {
        self.quota_preflight_total.add(
            1,
            &[
    ...
    fn record_quota_reserve(&self, period: &str) {
        self.quota_reserve_total
            .add(1, &[KeyValue::new("period", period.to_owned())]);
    }

    fn record_quota_commit(&self, period: &str) {
        self.quota_commit_total
            .add(1, &[KeyValue::new("period", period.to_owned())]);
    }

    fn record_quota_overshoot(&self, period: &str) {
        self.quota_overshoot_total
            .add(1, &[KeyValue::new("period", period.to_owned())]);
    }
    ...
    fn record_stream_incomplete(&self, provider: &str, model: &str, reason: &str) {
        self.stream_incomplete_total.add(
            1,
            &[
                KeyValue::new("provider", provider.to_owned()),
                KeyValue::new("model", model.to_owned()),
    ...
    fn record_cancel_requested(&self, trigger: &str) {
        self.cancel_requested_total
            .add(1, &[KeyValue::new("trigger", trigger.to_owned())]);
    }

    fn record_cancel_effective(&self, trigger: &str) {
        self.cancel_effective_total
            .add(1, &[KeyValue::new("trigger", trigger.to_owned())]);
    }

    fn record_time_to_abort_ms(&self, trigger: &str, ms: f64) {
        self.time_to_abort_ms
            .record(ms, &[KeyValue::new("trigger", trigger.to_owned())]);
    }
    ...
    fn record_attachment_upload(&self, kind: &str, result: &str) {
        self.attachment_upload_total.add(
            1,
            &[
    ...
    fn record_attachment_upload_bytes(&self, kind: &str, bytes: f64) {
        self.attachment_upload_bytes
            .record(bytes, &[KeyValue::new("kind", kind.to_owned())]);
    }
...
mod tests {
    ...
    fn stream_counters_increment() {
        ...
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_stream_started_total"),
            2
        );
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_stream_completed_total"),
            1
        );
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_stream_failed_total"),
            1
        );
    }
    ...
    fn turn_mutation_counter_increments() {
        ...
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_turn_mutation_total"),
            3
        );
    }
    ...
    fn quota_counters_increment() {
        ...
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_quota_preflight_total"),
            1
        );
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_quota_reserve_total"),
            1
        );
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_quota_commit_total"),
            1
        );
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_quota_overshoot_total"),
            1
        );
    }
    ...
    fn configurable_prefix() {
        ...
        assert_eq!(
            extract_counter_value(&exporter, "my_chat_stream_started_total"),
            1
        );
    ...
    fn cancellation_counters_increment() {
        ...
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_cancel_requested_total"),
            1
        );
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_cancel_effective_total"),
            1
        );
        assert_eq!(
            extract_counter_value(&exporter, "mini_chat_streams_aborted_total"),
            1
        );
    }
}
